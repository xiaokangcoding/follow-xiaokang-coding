...
各位小伙伴们！还记得我们上次聊的 `select` 吗？如果说 `select` 是 Linux I/O 复用机制的入门级角色，那今天我们要聊的 `poll` 就是升级版啦！可以更灵活处理成百上千的文件描述符，功能更强大，而且性能在很多场景下比 `select` 还要好。今天这篇文章，会让大家从“啥是 `poll`”到“会用 `poll`”，全程轻松有趣，不烧脑。咱们边聊边学，让这家伙从高冷的“技术名词”变成咱的老朋友。

> **友情提醒**：记得点赞、在看、转发支持我哦！多谢！🌟

## 一、poll 是个啥？和 select 有啥不一样？

还记得 `select` 吧？它的核心就是监控一堆文件描述符（比如套接字）看哪个有数据，谁“先出声”就处理谁。`poll` 也是干这个的，只不过改进了不少痛点。

### 1. select 的缺点回顾
+ **文件描述符数量有限**：`select` 能监听的文件描述符最多1024个，这要在高并发情况下就有点尴尬。
+ **效率问题**：每次都要把描述符数组重新“打包”给内核，性能不佳。
+ **繁琐**：每次都要设置 fd 集合，感觉重复而麻烦。

### 2. poll 的改进点
`poll` 针对 `select` 的缺陷做了优化：

+ **数量上不再受限**：理论上支持的文件描述符数量不限，满足咱的“放开了用”需求。但还是会受到系统级文件描述符数量的限制。
+ **使用结构体数组传递描述符**：不需要每次重置集合，效率提升。
+ **支持更多事件类型**：除了 `select` 的读写之外，还能监控错误 POLLERR 等事件，功能更全。

## 二、poll 怎么用？从 0 到 1 完整走一遍！

接下来，咱们来从代码上彻底搞懂 `poll` 怎么用！看完这一节，相信你会发现其实并不难。
